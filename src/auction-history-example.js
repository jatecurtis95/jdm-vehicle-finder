...
const CARS = [
  { id: "h1", make: "NISSAN", model: "SKYLINE", variant: "GT-R V-SPEC", code: "BNR34", year: 2000, km: 62000, grade: 4.5, house: "USS Tokyo", date: "2026-07-09", jpy: 12850000, aud: 135263, landed: 158900, image: "/assets/photo/web/hero_r32_garage.jpg", eligible: "25-year rule", transmission: "Manual", drivetrain: "AWD", engine: 2600, body: "Coupe", fuel: "Petrol", colour: "Bayside Blue" },
  { id: "h2", make: "NISSAN", model: "SKYLINE", variant: "GT-T", code: "ER34", year: 1999, km: 88000, grade: 4, house: "TAA Kinki", date: "2026-07-04", jpy: 4280000, aud: 45053, landed: 61200, image: "/assets/photo/web/r34_highway_bw.jpg", eligible: "25-year rule", transmission: "Automatic", drivetrain: "RWD", engine: 2500, body: "Coupe", fuel: "Petrol", colour: "Black" },
  { id: "h3", make: "TOYOTA", model: "SUPRA", variant: "RZ-S", code: "JZA80", year: 1997, km: 71000, grade: 4.5, house: "USS Nagoya", date: "2026-06-28", jpy: 9650000, aud: 101579, landed: 122400, image: "/assets/photo/web/tokyo_r34_night.jpg", eligible: "25-year rule", transmission: "Manual", drivetrain: "RWD", engine: 3000, body: "Coupe", fuel: "Petrol", colour: "White" },
  { id: "h4", make: "MAZDA", model: "RX-7", variant: "Type RS", code: "FD3S", year: 1999, km: 95000, grade: 4, house: "USS Osaka", date: "2026-06-21", jpy: 5820000, aud: 61263, landed: 78200, image: "/assets/photo/web/s14_garage.jpg", eligible: "25-year rule", transmission: "Manual", drivetrain: "RWD", engine: 1300, body: "Coupe", fuel: "Petrol", colour: "Silver" },
  { id: "h5", make: "HONDA", model: "CIVIC", variant: "Type R", code: "EK9", year: 1998, km: 110000, grade: 3.5, house: "USS Tokyo", date: "2026-06-14", jpy: 3410000, aud: 35895, landed: 50900, image: "/assets/photo/web/s15_enginebay.jpg", eligible: "25-year rule", transmission: "Manual", drivetrain: "FWD", engine: 1600, body: "Hatchback", fuel: "Petrol", colour: "White" },
  { id: "h6", make: "TOYOTA", model: "CROWN", variant: "Royal Saloon G", code: "JZS155", year: 1998, km: 83000, grade: 4, house: "JU Tokyo", date: "2026-06-08", jpy: 1180000, aud: 12421, landed: 26100, image: "/assets/photo/web/shibuya_night.jpg", eligible: "25-year rule", transmission: "Automatic", drivetrain: "RWD", engine: 3000, body: "Sedan", fuel: "Petrol", colour: "Pearl" },
];

const money = (n) => new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD", maximumFractionDigits: 0 }).format(n);
const yen = (n) => `JPY ¥${new Intl.NumberFormat("en-AU").format(n)}`;
...
function filters(params) {
  ...
  const mileageMax = Number(params.mileageMax) || Infinity;
  const engineMin = Number(params.engineMin) || 0;
  const engineMax = Number(params.engineMax) || Infinity;
  const exact = (key, value) => !value || String(key).toUpperCase() === String(value).trim().toUpperCase();
  const rangeDays = { "4w": 28, "6w": 42, "3m": 92, "6m": 183, "12m": 366 }[params.range] || 0;
  ...
  return CARS.filter((car) => {
    ...
    return (!q || haystack.includes(q)) && (!make || car.make === make) && car.year >= yearMin && car.year <= yearMax && car.grade >= gradeMin &&
      car.km <= mileageMax && car.engine >= engineMin && car.engine <= engineMax && (!cutoff || new Date(`${car.date}T00:00:00Z`).getTime() >= cutoff) &&
      exact(car.transmission, params.transmission) && exact(car.house, params.house) && exact(car.body, params.body) &&
      exact(car.drivetrain, params.drivetrain) && exact(car.fuel, params.fuel) && exact(car.colour, params.colour) &&
      (!params.eligibility || params.eligibility === "Eligible");
  });
}
...
function resultRow(car) {
  ...
  return `<tr>
    ...
    <td><strong>${num(car.km)} km</strong><span>Grade ${car.grade} · ${car.transmission}</span></td>
...
function resultCard(car) {
  ...
  return `<article class="ah-result-card">
    ...
    <dl><div><dt>Odometer</dt><dd>${num(car.km)} km</dd></div><div><dt>Gearbox / drive</dt><dd>${car.transmission} · ${car.drivetrain}</dd></div><div><dt>Hammer</dt><dd>${yen(car.jpy)}</dd></div><div><dt>Est. landed</dt><dd class="ah-landed">${money(car.landed)}</dd></div></dl>
...
export function auctionHistoryExamplePage(params = {}) {
  ...
  const rangeLabels = { "4w": "4 weeks", "6w": "6 weeks", "3m": "3 months", "6m": "6 months", "12m": "12 months" };
  if (params.range) active.push(chip(rangeLabels[params.range] || params.range, "range", params));
  for (const [key, label] of [["transmission", "Transmission"], ["mileageMax", "Under"], ["engineMin", "Engine from"], ["engineMax", "Engine to"], ["house", "Auction"], ["body", "Body"], ["drivetrain", "Drive"], ["fuel", "Fuel"], ["colour", "Colour"], ["eligibility", "Eligibility"]]) {
    if (params[key]) active.push(chip(`${label}: ${params[key]}${key === "mileageMax" ? " km" : key.startsWith("engine") ? " cc" : ""}`, key, params));
  }
  const advancedOpen = ["transmission", "mileageMax", "engineMin", "engineMax", "house", "body", "drivetrain", "fuel", "colour", "eligibility"].some((k) => params[k]);
  ...
  const content = `<style>${CSS}</style><div class="ah-shell">
    ...
    <main id="main">
      ...
      <form class="ah-filter" action="/auction-history-example" method="GET">
        ...
        <fieldset class="ah-range"><legend>Auction held within</legend>${[["4w", "4 weeks"], ["6w", "6 weeks"], ["3m", "3 months"], ["6m", "6 months"], ["12m", "12 months"]].map(([value, label]) => `<label><input type="radio" name="range" value="${value}"${params.range === value ? " checked" : ""}><span>${label}</span></label>`).join("")}</fieldset>
        <details class="ah-more"${advancedOpen ? " open" : ""}><summary><span>More filters</span><small>Transmission, mileage, engine, auction house and more</small></summary><div class="ah-more-grid">
          <label><span>Transmission</span><select name="transmission">${option("", "Any transmission", params.transmission)}${option("Manual", "Manual", params.transmission)}${option("Automatic", "Automatic", params.transmission)}${option("CVT", "CVT", params.transmission)}</select></label>
          <label><span>Maximum mileage</span><input type="number" name="mileageMax" min="0" step="1000" value="${esc(params.mileageMax || "")}" placeholder="100000"></label>
          <label><span>Engine from (cc)</span><input type="number" name="engineMin" min="0" step="100" value="${esc(params.engineMin || "")}" placeholder="1500"></label>
          <label><span>Engine to (cc)</span><input type="number" name="engineMax" min="0" step="100" value="${esc(params.engineMax || "")}" placeholder="3000"></label>
          <label><span>Auction house</span><select name="house">${option("", "All auction houses", params.house)}${["USS Tokyo", "TAA Kinki", "USS Nagoya", "USS Osaka", "JU Tokyo"].map((x) => option(x, x, params.house)).join("")}</select></label>
          <label><span>Body type</span><select name="body">${option("", "Any body", params.body)}${["Coupe", "Sedan", "Hatchback"].map((x) => option(x, x, params.body)).join("")}</select></label>
          <label><span>Drivetrain</span><select name="drivetrain">${option("", "Any drivetrain", params.drivetrain)}${["RWD", "AWD", "4WD", "FWD"].map((x) => option(x, x, params.drivetrain)).join("")}</select></label>
          <label><span>Fuel</span><select name="fuel">${option("", "Any fuel", params.fuel)}${["Petrol", "Diesel", "Hybrid", "Electric"].map((x) => option(x, x, params.fuel)).join("")}</select></label>
          <label><span>Colour</span><select name="colour">${option("", "Any colour", params.colour)}${["Bayside Blue", "Black", "White", "Silver", "Pearl"].map((x) => option(x, x, params.colour)).join("")}</select></label>
          <label><span>Import eligibility</span><select name="eligibility">${option("", "Any status", params.eligibility)}${option("Eligible", "Eligible for Australia", params.eligibility)}</select></label>
        </div></details>
      </form>
      ...
      <section class="ah-snapshot" aria-labelledby="snapshot-title"><div><span class="ah-kicker">Recent market snapshot</span><h2 id="snapshot-title">${rows.length} comparable sale${rows.length === 1 ? "" : "s"}</h2><p>Demo results based on the filters above. Production would use the live sold-auction feed.</p></div>
        <dl><div><dt>Median hammer (AUD)</dt><dd>${money(median)}</dd></div><div><dt>Observed range (AUD)</dt><dd>${rows.length ? `${money(min)}–${money(max)}` : "—"}</dd></div><div><dt>Typical grade</dt><dd>${rows.length ? (rows.reduce((s, x) => s + x.grade, 0) / rows.length).toFixed(1) : "—"}</dd></div></dl>
      </section>
...
const CSS = `
  ...
  body{background:var(--ah-paper);color:var(--ah-ink)}.ah-shell{min-height:100vh}.ah-nav{height:76px;padding:0 clamp(20px,5vw,72px);display:flex;align-items:center;gap:24px;background:#0d0f13;color:#fff;border-bottom:1px solid rgba(255,255,255,.1)}.ah-nav svg{width:210px;height:auto}.ah-nav svg path,.ah-nav svg polygon{fill:#fff}.ah-nav>span{font-size:11px;text-transform:uppercase;letter-spacing:.16em;color:#9ca0a8;border-left:1px solid #333740;padding-left:24px}.ah-nav>a:last-child{margin-left:auto;background:var(--ah-gold);color:#17130a;padding:11px 18px;border-radius:7px;font-weight:700;font-size:13px}
  .ah-shell main{max-width:1500px;margin:auto;padding:48px clamp(20px,5vw,72px) 80px}.ah-hero{display:flex;justify-content:space-between;align-items:flex-end;gap:32px;margin-bottom:32px}.ah-kicker{display:block;color:#8a6a22;font-size:11px;font-weight:800;letter-spacing:.15em;text-transform:uppercase;margin-bottom:9px}.ah-hero h1{font-size:clamp(38px,5vw,66px);line-height:.95;margin:0;letter-spacing:-.055em;color:var(--ah-ink)}.ah-hero p{font-size:17px;color:var(--ah-muted);max-width:650px;margin:16px 0 0}.ah-rate{background:#17191d;color:#fff;border-left:4px solid var(--ah-gold);padding:18px 22px;min-width:210px}.ah-rate span,.ah-rate small{display:block;color:#aeb1b7;font-size:11px}.ah-rate strong{display:block;font-size:21px;margin:3px 0}
  ...
  .ah-range{grid-column:1/-1;border:0;border-top:1px solid var(--ah-line);margin:5px 0 0;padding:15px 0 0;display:flex;align-items:center;gap:8px;flex-wrap:wrap}.ah-range legend{float:left;margin:7px 14px 0 0;font-size:11px;font-weight:800;color:#555a62}.ah-range label{position:relative;margin:0;cursor:pointer}.ah-range input{position:absolute;opacity:0;width:1px;height:1px}.ah-range span{display:block!important;margin:0!important;padding:8px 12px;border:1px solid #d8d6d0;border-radius:999px;background:#fbfbfa}.ah-range input:checked+span{background:#17191d;border-color:#17191d;color:#fff}.ah-range input:focus-visible+span{outline:3px solid rgba(202,163,76,.35);outline-offset:2px}
  .ah-more{grid-column:1/-1;border-top:1px solid var(--ah-line);padding-top:14px}.ah-more summary{list-style:none;cursor:pointer;display:flex;align-items:center;gap:12px;color:var(--ah-ink);font-size:13px;font-weight:800}.ah-more summary::-webkit-details-marker{display:none}.ah-more summary:after{content:"+";margin-left:auto;color:#8a6a22;font-size:22px}.ah-more[open] summary:after{content:"−"}.ah-more summary small{font-weight:500;color:var(--ah-muted)}.ah-more-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin-top:16px;padding:16px;background:#f7f6f3;border-radius:9px}
  ...
  @media(max-width:1050px){.ah-filter{grid-template-columns:2fr repeat(2,1fr)}.ah-filter button{width:100%}.ah-more-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.ah-snapshot{grid-template-columns:1fr}.ah-table{display:none}.ah-mobile{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;padding:0 18px 18px}.ah-result-card{border:1px solid var(--ah-line);border-radius:10px;overflow:hidden;background:#fff}.ah-result-card>img{width:100%;height:210px;object-fit:cover}.ah-rc-body{padding:16px}.ah-rc-head{display:flex;justify-content:space-between;gap:12px}.ah-rc-head span{font-size:10px;letter-spacing:.08em;color:#8b6a20}.ah-rc-head h2{font-size:17px;margin:3px 0}.ah-rc-head p{font-size:12px;color:var(--ah-muted);margin:0}.ah-rc-head>b{border:2px solid var(--ah-gold);border-radius:50%;width:42px;height:42px;display:grid;place-items:center}.ah-result-card dl{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:18px 0}.ah-result-card dt{font-size:10px;color:var(--ah-muted)}.ah-result-card dd{font-size:13px;font-weight:700;margin:3px 0 0}.ah-rc-foot{display:flex;justify-content:space-between;gap:12px;align-items:center;border-top:1px solid var(--ah-line);padding-top:13px}.ah-rc-foot span{font-size:10px;color:#31714e}.ah-rc-foot a{font-size:12px;color:#755510;font-weight:800}}
  @media(max-width:680px){.ah-nav{height:64px;padding:0 16px}.ah-nav svg{width:160px}.ah-nav>span{display:none}.ah-nav>a:last-child{padding:9px 11px}.ah-shell main{padding:30px 14px 60px}.ah-hero{align-items:flex-start;flex-direction:column}.ah-rate{width:100%}.ah-filter{grid-template-columns:1fr 1fr;padding:14px}.ah-query,.ah-filter button{grid-column:1/-1}.ah-range{display:grid;grid-template-columns:repeat(3,1fr)}.ah-range legend{float:none;grid-column:1/-1;margin:0}.ah-range span{text-align:center;padding:8px 5px}.ah-more summary{align-items:flex-start;flex-direction:column;gap:2px;position:relative;padding-right:24px}.ah-more summary:after{position:absolute;right:0;top:-7px}.ah-more-grid{grid-template-columns:1fr 1fr;padding:12px}.ah-snapshot{padding:22px 18px}.ah-snapshot dl{grid-template-columns:1fr}.ah-snapshot dl div{border-left:0;border-top:1px solid #34373d;padding:12px 0}.ah-mobile{grid-template-columns:1fr;padding:0 12px 12px}.ah-results-head{padding:20px 16px}.ah-result-card>img{height:190px}}
`;
