...
import {
  ...
  HISTORY_RANGES, HISTORY_SORTS, LIVE_SORTS, HISTORY_RATES, RATE_ORDER,
...
function filterForm(p, { makers, models, codes, houses, grades }, surface, opts = {}) {
  ...
  // dropdown dance. Canonical pill order R, RA, 2, 3, 3.5, 4, 4.5, 5, 6, S.
  const rateSet = new Set(p.rates ? p.rates.split(",") : []);
  const ratePills = RATE_ORDER.map((k) =>
