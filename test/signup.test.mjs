...
test("self-signup is refused for an email that already exists (takeover prevention)", async () => {
  ...
  const before = env.DB.prepare("SELECT id, pass_hash FROM users WHERE email = ?").bind("victim@example.com").first();
  ...
  const after = env.DB.prepare("SELECT pass_hash, portal_enabled FROM users WHERE id = ?").bind(before.id).first();
...
test("self-signup creates a login for a brand-new client", async () => {
  ...
  const c = env.DB.prepare("SELECT pass_hash, portal_enabled FROM users WHERE email = ?").bind("new@example.com").first();
...
test("signup now requires a mobile number (the form makes it mandatory)", async () => {
  ...
  assert.equal((await env.DB.prepare("SELECT COUNT(*) AS n FROM users").first()).n, 0, "nothing created without a phone");
});
...
test("the request form now requires a make and a model", async () => {
  ...
  assert.equal((await env.DB.prepare("SELECT COUNT(*) AS n FROM users").first()).n, 0, "nothing created");
});
...
test("a second signup on an email that already has a login folds in neutrally (V1.2 Phase 3)", async () => {
  ...
  const before = env.DB.prepare("SELECT id, pass_hash FROM users WHERE lower(email)='dupe@example.com'").first();
  ...
  const rows = (await env.DB.prepare("SELECT id, pass_hash FROM users WHERE lower(email)='dupe@example.com'").all()).results;
