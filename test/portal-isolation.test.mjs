...
const FIX = `
  ...
  INSERT INTO wishlists (id,client_id,marka_name) VALUES (1,1,'TOYOTA'),(2,2,'NISSAN');
...
test("a client cannot edit another client's saved search", async () => {
  ...
  assert.equal(env.db.prepare("SELECT marka_name FROM wishlists WHERE id = 1").get().marka_name, "TOYOTA", "client 1's search is untouched");
});
...
test("a client cannot pause or delete another client's saved search", async () => {
  ...
  assert.equal(env.db.prepare("SELECT active FROM wishlists WHERE id = 1").get().active, 1, "still active");
  await portalDeleteWishlist(env, fd({ id: "1" }), { role: "client", id: 2 });
  assert.ok(env.db.prepare("SELECT 1 FROM wishlists WHERE id = 1").get(), "still exists");
  // And the owner still can: the guard blocks strangers, not the owner.
  await portalDeleteWishlist(env, fd({ id: "1" }), { role: "client", id: 1 });
  assert.equal(env.db.prepare("SELECT 1 FROM wishlists WHERE id = 1").get(), undefined, "owner delete works");
});
