...
test("one-step create requires a name, but contact is optional for staff", async () => {
  ...
  assert.equal((await env.DB.prepare("SELECT COUNT(*) AS n FROM users").first()).n, 1, "the contactless customer is stored");
  const c = await env.DB.prepare("SELECT name, email, whatsapp FROM users WHERE id=?").bind(r.clientId).first();
